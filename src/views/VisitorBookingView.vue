...
<template>
  <main class="visitor-booking" dir="rtl">
    ...
    <section v-else-if="step === 'details'" class="booking-layout content"><div class="booking-card"><button class="back" @click="backToDoctors"><v-icon icon="mdi-arrow-right" /> العودة للأطباء</button><h2>تفاصيل الحجز</h2><p class="muted">{{ selectedDoctor?.name }} · {{ selectedDoctor?.specializationName }}</p><div class="field"><label>العيادة</label><select v-model="selectedClinic" @change="loadDays"><option v-for="clinic in selectedDoctor?.clinics" :key="clinic.id" :value="clinic">{{ clinic.name }} — {{ clinic.iraqiProvinceName }}</option></select></div><div class="field"><label>اختر اليوم المتاح</label><div v-if="loadingDays" class="mini-loading"><v-progress-circular indeterminate size="25" /></div><div v-else class="days"><button v-for="day in days" :key="day.date" :disabled="!day.isAvailable" :class="{ selected: selectedDay?.date === day.date }" @click="selectedDay = day"><b>{{ day.dayName }}</b><span>{{ dateLabel(day.date) }}</span><small v-if="day.isAvailable">{{ day.remainingAppointments }} موعد متاح</small><small v-else>{{ day.closureReason || 'غير متاح' }}</small></button></div></div></div><form class="booking-card" @submit.prevent="createBooking"><h2>بيانات المراجع</h2><div class="field"><label>الاسم الكامل</label><input v-model="form.name" required placeholder="اكتب الاسم الثلاثي" /></div><div class="field"><label>رقم الهاتف</label><input v-model="form.phone" v-iraqi-phone dir="ltr" required placeholder="07XXXXXXXXX" /></div><div class="field"><label>ملاحظات للحجز <em>اختياري</em></label><textarea v-model="form.notes" rows="3" placeholder="أي ملاحظة تريد إضافتها"></textarea></div><div class="summary" v-if="selectedDay"><span>موعدك المختار</span><b>{{ selectedDay.dayName }}، {{ dateLabel(selectedDay.date) }} {{ time(selectedDay.startTime) ? ` · ${time(selectedDay.startTime)}` : '' }}</b></div><v-btn type="submit" block size="large" color="primary" :disabled="!canContinue" :loading="saving">متابعة وتأكيد الحجز</v-btn></form></section>
